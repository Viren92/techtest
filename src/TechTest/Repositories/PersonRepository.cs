...
namespace TechTest.Repositories
{
    public class PersonRepository : IPersonRepository
    {
        ...
        {
            var person = Collection.SingleOrDefault(p => p.Id == id);
        ...
        public Person Update(Person person)
        {
            ...
            var existing = Collection.SingleOrDefault(p => p.Id == person.Id);

            if (existing != null)
            {
                Collection = Collection.Except(new List<Person> { existing }).ToList();
